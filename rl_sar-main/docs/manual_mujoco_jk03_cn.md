...
默认还会添加一个临时平地场景，让 JK03 站在地面上，而不是在空场景里漂浮或下落。
...
# 按 rl_sar-main/src/rl_sar/worlds/stairs.world 的台阶尺寸生成楼梯
...
ground        平地，默认值
low-stairs    低台阶
stairs-world  参考仓库 Gazebo stairs.world 的楼梯尺寸
```

默认启用了软平衡辅助，避免未训练控制器马上翻倒。它会根据台阶高度轻微调整目标机身高度，但它不是训练出来的策略，也不适合实机；高楼梯能否稳定通过仍然取决于后续策略和控制效果。
